...
RentID INT PRIMARY KEY not null,
TenantID int not null,ApartmentID int not null,
Termfrom date not null,Termupto date not null,
foreign key(TenantID)references tenant(TenantID),foreign key(ApartmentID)references apartment(ApartmentID)
)
create table counts(CountsID INT PRIMARY KEY not null,
CountsSumma int not null,CountsData date not null,
CountsPenia int not null,TenantID int not null,
foreign key(TenantID)references tenant(TenantID))